...
ENTITY rolled_Encryption_v2 IS
PORT(
...
END rolled_Encryption_v2;

ARCHITECTURE Behavioral OF rolled_Encryption_v2 IS
...
-- Signals for wd_16 inputs, outputs, and intermediate results
    ...
    BEGIN
        ...
        y1_out: rolled_chi_nu PORT MAP(clk => clk, rst => rst, start => start_enc, x_in => xn_input_1, key => key(127 DOWNTO 64), y_out => y1, done => done_1);

        xn_input_2 <= STD_LOGIC_VECTOR((UNSIGNED(R_i(111 DOWNTO 96)) + UNSIGNED(y1)) MOD 65536);
        y2_out: rolled_chi_nu PORT MAP(clk => clk, rst => rst, start => done_1, x_in => xn_input_2, key => KxR_1, y_out => y2, done => done_2);

        xn_input_3 <= STD_LOGIC_VECTOR((UNSIGNED(R_i(95 DOWNTO 80)) + UNSIGNED(y2)) MOD 65536);
        y3_out: rolled_chi_nu PORT MAP(clk => clk, rst => rst, start => done_2, x_in => xn_input_3, key => KxR_2, y_out => y3, done => done_3);

        xn_input_4 <= STD_LOGIC_VECTOR((UNSIGNED(R_i(79 DOWNTO 64)) + UNSIGNED(y3)) MOD 65536);
        y4_out: rolled_chi_nu PORT MAP(clk => clk, rst => rst, start => done_3, x_in => xn_input_4, key => key(63 DOWNTO 0), y_out => y4, done => done_4);
